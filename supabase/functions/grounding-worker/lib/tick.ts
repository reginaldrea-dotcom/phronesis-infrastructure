...
function groundingPrompt(claimId: string, hint: Record<string, unknown> | null): string {
  ...
  const lines = [
    ...
    "   - For a QUALITATIVE claim: omit claim_canonical_string; pass anchor_quote = the shortest span that makes the claim. IT MUST BE A VERBATIM EXTRACT - copied word-for-word from the captured page's own text, NEVER a paraphrase, reconstruction, reorder, or a stitch across two sentences. CHECK 1 now runs on prose exactly as on figures: the edge reaches SCREENSHOT_REVIEW only if that span is present verbatim on the captured page; otherwise it is cited_not_verified (a human never sees it as reviewable). NO QUOTE, NO reviewable edge. So: find the exact sentence on the page and copy it; do not compose one from the claim.",
    "CAPTURE THE LOCUS, NOT THE ROOT: capture the specific page/section that STATES the claim - the exact PDF page, the section page - never a contents/FAQ/catalogue/landing page that only links to it (that page will not contain the quote and Check 1 will correctly reject it). This is the C12 index rule, one level finer.",
  ];
  if (docId) {
